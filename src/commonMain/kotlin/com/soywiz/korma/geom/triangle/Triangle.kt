...
fun ITriangle.point(index: Int) = when (index) {
    0 -> p0
    1 -> p1
    2 -> p2
 ...
 */
fun ITriangle.containsPoint(point: Point2d): Boolean = (point == p0) || (point == p1) || (point == p2)
/**
...
fun ITriangle.containsEdge(edge: Edge): Boolean = containsEdgePoints(edge.p, edge.q)

// In a triangle to check if contains and edge is enough to check if it contains the two vertices.
fun ITriangle.containsEdgePoints(p1: Point2d, p2: Point2d): Boolean = containsPoint(p1) && containsPoint(p2)

private fun _product(p1: Point2d, p2: Point2d, p3: Point2d): Double =
    (p1.x - p3.x) * (p2.y - p3.y) - (p1.y - p3.y) * (p2.x - p3.x)

fun ITriangle.pointInsideTriangle(pp: Point2d): Boolean = if (_product(p0, p1, p2) >= 0) {
