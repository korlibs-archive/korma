...
fun <T> Float.interpolateAny(min: T, max: T): T = when (min) {
    ...
    is Interpolable<*> -> (min as Interpolable<Any>).interpolateWith(max as Interpolable<Any>, this) as T
    else -> throw IllegalArgumentException("Value is not interpolable")
}

fun <T> Double.interpolateAny(min: T, max: T): T = this.toFloat().interpolateAny(min, max)
